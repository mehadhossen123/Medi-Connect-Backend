...
//   send otp into redis
  ...
  // here data save into redis and then get otp and match
  // redis payload
  const redisPayload = {
    name,
    email,
  ...
  const patientRegisterKey = `patient-register-key:${email}`;

  await client.set(patientRegisterKey, JSON.stringify(redisPayload), {
    expiration: {
      type: "EX",
      value: 5 * 60,
    },
  ...
  const templatePath = path.join(
    process.cwd(),
    "/src/app/templete/register.otp.ejs",
  );
  ...
  await transporter.sendMail({
    ...
    to:email,
    subject: "MediConnect Email verified otp",
    html,
  });
...
const loginUser = async (payload: ILoginUserPayload) => {
  ...
  if (user.password == null && user.googleId !== null) {
    throw new Error(
      "User already exist! Registered with google . please try to google login ",
    );
  }
...
//   send email
  const templatePath = path.join(
    process.cwd(),
    "/src/app/templete/forgot-templete.ejs",
  );
...
const emailVerifyForPatient=async(payload:IEmailVerifyPayload)=>{
  ...
  const getOtp = await client.get(otpKey);
  ...
  const patientRegisterKey = `patient-register-key:${email}`;
  const getRedisPayloadData = await client.get(patientRegisterKey);

  if (!getRedisPayloadData) {
    throw new Error("Doesn't exist the patient into redis");
  }

  const payloadUser:IRegisterPatientPayload = JSON.parse(getRedisPayloadData);
    ...
    const createdUser = await prisma.user.create({
      data: {
        ...
        emailVerified:true,
        patient: {
          create: { name: payloadUser.name, email: payloadUser.email ,
			contactNumber:payloadUser?.patient?.contactNumber || ""
		  },
        },
      },
      omit: { password: true },
